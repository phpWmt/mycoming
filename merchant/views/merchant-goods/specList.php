                            ...
                            <?php if (is_array($goods_spec)): foreach ($goods_spec as $k => $v): ?>
                                <tr>
                                    ...
                                    <td align="center">
                                        ...
                                        <?php if($v["status"] == 0):?>
                                            <a class="btn btn-primary btn-rounded btn-outline btn-xs" href="<?php echo Url::to(['merchant-goods/spec-status','id'=>$v['id'],'status'=>1,'goods_id'=>$goods_id])?>"><i class="fa fa-check"></i>&nbsp;启用</a>
                                        <?php else:?>
                                            <a class="btn btn-danger btn-rounded btn-outline btn-xs" href="<?php echo Url::to(['merchant-goods/spec-status','id'=>$v['id'],'status'=>0,'goods_id'=>$goods_id])?>"><i class="glyphicon glyphicon-remove"></i>&nbsp;禁用</a>
                                        <?php endif;?>
...
$js=<<<JS
        ...
        $(".del").click(function () {
            ...
            var d = dialog({
                ...
                ok: function () {
                    $.ajax({
                        url: "/merchant/merchant-goods/goods-delete-spec",
                        type: "post",
                        dataType: "json",
                        data:{
                            'id':id,
  ...
  $(".goods").click(function() {
    ...
    $("#add").click(function() {
    $(".myform").ajaxSubmit({
        url:"/merchant/merchant-goods/add-permissions",
        type:"post",
        dataType:"json",
         success: function (data) {
            if(data.status === 1) {
