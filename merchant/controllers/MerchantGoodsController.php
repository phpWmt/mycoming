...
use merchant\models\Spec; //商品规格表
use merchant\models\AloneSpec; //子商品规格表
use merchant\models\GodownType;//库存类型
...
class MerchantGoodsController extends CommonController {
    use TreeTrait;
        ...
        return [
            'upload' => [
                ...
                'config' => [
                    ...
                    "fileRoot" => Yii::getAlias("@merchant").'/web',
                ],
            ]
        ];
    }
    ...
    public function actionList(){
        ...
        $model->andWhere(['dyd_goods.entrepot'=>$entrepotId]);
...
//        echo $commandQuery->createCommand()->getRawSql();
    ...
    public function actionCreate(){
        ...
        if ($request->isPost){
            ...
            if (!empty($post['paramName'])){
                ...
                //添加商品
                $model =new Goods();
                $model->entrepot=Yii::$app->admin->getIdentity()['entrepot'];
                ...
                //返回添加商品规格
                if ($result){
                    return ['status'=>1,'info'=>'添加成功,请添加商品规格！','url'=>Url::to(['merchant-goods/add-spec?id='.$id])];
                }
            }
        }else{
            $model=new Goods();
    ...
    public function actionAddSpec(){
        ...
        if($request->isGet){
            ...
            //返回
            if ($result){
                return ['status'=>1,'info'=>'添加成功','url'=>Url::to(['merchant-goods/add-list?id='.$goodsId])];
            }
        }
    }
    ...
    public function actionAddList(){
        ...
        if($request->isGet) {
            ...
            //返回
            if ($result){
                return ['status'=>1,'info'=>'商品规格添加成功','url'=>Url::to(['merchant-goods/list'])];
            }
        }
    }
    ...
    public function actionRemove(){
        ...
        if ($result){
            ...
            return $this->redirect(['merchant-goods/add-spec?id='.$id]);
        }
    ...
    public function actionSpecStatus(){
        ...
        if(Yii::$app->request->isGet){
            ...
            if($resule){
                ...
                return $this->redirect(['merchant-goods/spec-list?id='.$get['goods_id']]);
            }
    ...
    public function actionGodown(){
        ...
        if ($request->isPost){
            ...
            //入库库存总数不能大于商品剩余库存
            if($num > $repertory){
                return ['status'=>2,'info'=>"<B style='text-align:center;color: red'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;入库失败,该商品库存不足!</B></br>剩余库存:<B>$repertory</B>.入库总数为:<B>$num</B></br>"];die;
            }
            ...
            $entrepotId =  Yii::$app->admin->getIdentity()['entrepot'];
            //查询出仓库名
            $entrepotName = Entrepot::find()->where(['id'=>$entrepotId])->select('entrepot_name')->asArray()->one();

            //判断仓库是否已入库该商品
            $goodRersult = EntrepotGoods::find()->where(['entrepot_id'=>$entrepotId,'goods_id'=>$post['goods_id']])->select('id')->asArray()->one();

            //限制重复入库
            if(empty($goodRersult)){
                //商品入库
                $result = $this->entrepotGoods($entrepotId, $post, $arr, $num, $arrSpec);
            }else{
            ...
            //返回
            if ($result) {
                return ['status'=>1,'info'=>'商品入库成功','url'=>Url::to(['merchant-goods/list'])];
            } else {
     ...
     *  @$arrSpec   一级规格
     */
    public function entrepotGoods($entrepotId,$post,$spec,$totalNum,$arrSpec){
        ...
        if(!empty($goods_id)){
            ...
            //添加子规格
            $this->entrepotGoodsSpec($goods_id,$entrepotId,$spec);
     ...
     * @param $spec    规格数据
     */
    public function entrepotGoodsSpec($id,$entrepotId,$spec){
        //实例化
    ...
    public function actionListDetail(){
        ...
        $get=Yii::$app->request->get();
        if (!empty($get['keyword'])){
            $model->andWhere(['like','title',$get['keyword']]);
        }
        if (!empty($get['start_time'])&&!empty($get['end_time'])){
        ...
        $model->andWhere(['dyd_godown_detail.entrepot_id'=>$entrepotId]);

        $model->joinWith('goods');  //商品表G
    ...
    public function actionGoodsInventory(){
        ...
        $model->andWhere(['dyd_entrepot_goods_spec.entrepot_id'=>$entrepotId]);
    ...
    public function actionSpecIncrease(){
        ...
        if(Yii::$app->request->isPost){
            ...
            if($res){
                ...
                return $this->redirect(['merchant-goods/goods-inventory']);
            }
...
/*************************************************************************************************************************************************/
    ...
    public function actionUpload($name){
        ...
        foreach ($uploadedFile as $k=>$v){
            ...
            $ymd = date("Ymd");
            //存储到本地的路径
            $save_path = \Yii::getAlias('@api') . '/web/uploads/merchant_img/' . $ymd .'/';
            //存储到数据库的地址
            $save_url = '/uploads' . '/merchant_img/' . $ymd . '/';
...
//        echo $commandQuery->createCommand()->getRawSql();
    ...
    public function actionDeleteImg(){
            $post=Yii::$app->request->post();
            @unlink(Yii::getAlias('@api').'/web'.$post['url']);
            $filedir  = dirname($post['url']);
            $files = @scandir(Yii::getAlias('@api').'/web'.$filedir);
            if (count($files)<=2) {
                @rmdir(Yii::getAlias('@api').'/web'.$filedir);//如果是./和../,直接删除文件夹
            }
            Yii::$app->response->format=Response::FORMAT_JSON;
            return ['status'=>1];
    }
    ...
    public function actionUpdate(){
        ...
        if ($request->isPost){
            ...
            if (!empty($post['paramName'])){
                ...
                $model->cover=str_replace("$api","",implode(',',array_filter($post['cover'])));
                $model->img=str_replace("$api","",implode(',',array_filter($post['img'])));
                ...
                $model->create_time=date('Y-m-d H:i:s',time());
                if ($model->save(false)){
                    return ['status'=>1,'info'=>'修改成功','url'=>Url::to(['merchant-goods/list'])];
                }
            }
        }else{
            $id=$request->get('id');
    ...
    private static function thumbnail($image, $width, $height, $mode = ManipulatorInterface::THUMBNAIL_OUTBOUND){
       Image::thumbnail($image,100,100,ManipulatorInterface::THUMBNAIL_INSET)
           ->save(Yii::getAlias('@merchant/web/uploads/test.jpg'),['quality' => 100]);
    }
     ...
     */
    private static function text($image, $text, $fontFile='@merchant/web/fonts/msyh.ttc',$start=[10,10],$fontOptions=['size'=>15,'color'=>'1BB394']){
        Image::text($image,'老司机666',$fontFile,$start,$fontOptions)
            //quality 图片生成质量
            ->save(Yii::getAlias('@merchant/web/uploads/text.jpg'),['quality' => 100]);
    }
    ...
    private static function watermark($image, $watermarkImage, $start = [0, 0]){
        Image::watermark($image,$watermarkImage, $start)
            ->save('@merchant/web/uploads/watermark.jpg',['quality' => 100]);
    }
    ...
    private static function crop($image, $width, $height, $start = [0, 0]){
        Image::crop($image, $width, $height, $start)
            ->save('@merchant/web/uploads/crop.jpg',['quality' => 100]);
    }
