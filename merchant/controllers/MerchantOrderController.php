...
use merchant\models\EntrepotGoods;
use merchant\models\Goods;
use merchant\models\Order;
use merchant\models\OrderDetail;
use merchant\models\OrderRecord; //发货记录
use merchant\models\OrderPay; //收款记录
use merchant\models\EntrepotGoodsSpec;//仓库规格ID
use merchant\models\OrderDetailed;//订单出入库明细
use merchant\models\Comment;//订单评价
...
Class MerchantOrderController extends CommonController
{
    protected $request;
    ...
    public function actionIndex()
    {
        ...
        //仓库ID
        $order->andWhere(['dyd_order.user_del' => '0','entrepot_id'=> Yii::$app->admin->getIdentity()['entrepot']]);
    ...
    public function actionOrderRecord(){
        ...
        if($resOrder){
            ...
            if ($res1 && $res2 && $res3){
                ...
                return ['status'=>1,'info'=>'发货成功！','url'=>Url::to(['merchant-order/index'])];
            }
    ...
    public function actionComment(){
       ...
       $entrepotId =  Yii::$app->admin->getIdentity()['entrepot'];
        ...
        //订单评价
        $Comment->andWhere(['dyd_comment.entrepot_id'=>$entrepotId]); //条件
        $Comment->joinWith('user'); //用户
        $Comment->joinWith('goods'); //商品
        $Comment->joinWith('entrepot'); //仓库
        //出入库明细
