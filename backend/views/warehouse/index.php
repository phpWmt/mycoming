                        ...
                        <div class="col-sm-12 p-m">
                            <form method="get">
                                <div class="input-group list-group col-sm-2">
                                    <input type="text" name="keyword" placeholder="请输入仓库名称检索"
                        ...
                        <table class="table table-bordered">
                            ...
                            <?php if (is_array($list)): foreach ($list as $k => $v): ?>
                                <tr>
                                    ...
                                    <td align="center"><b><?php echo($v["entrepot_num"]); ?></b></td>
                                    <td align="center"><?php echo($v["entrepot_address"]); ?></td>


                                    <td align="center"><?php echo($v["add_time"]); ?></td>

                                    <td align="center"> <a class="btn btn-success btn-rounded btn-outline btn-xs" href="<?php echo Url::to(['warehouse/inventory','id'=>$v['id'],'name'=>$v["entrepot_name"],'status'=>$v['status']])?>"><i class="glyphicon glyphicon-eye-open"></i>&nbsp;查看库存</a></td>
                                    ...
                                    <td align="center">
                                        <?php if($v["status"] == 0):?>
                                            <a class="btn btn-primary btn-rounded btn-outline btn-xs" href="<?php echo Url::to(['warehouse/status','id'=>$v['id'],'status'=>1])?>"><i class="fa fa-check"></i>&nbsp;启用</a>
                                        <?php else:?>
                                            <a class="btn btn-danger btn-rounded btn-outline btn-xs" href="<?php echo Url::to(['warehouse/status','id'=>$v['id'],'status'=>0])?>"><i class="glyphicon glyphicon-remove"></i>&nbsp;禁用</a>
                                        <?php endif;?>

                                        &nbsp;&nbsp;&nbsp;&nbsp;

                                        <a href="<?php echo Url::to(['warehouse/editor','id'=>$v['id']]) ?>">
...
$js=<<<JS
        ...
        $(".del").click(function () {
            ...
            var d = dialog({
                title: '友情提示',
                id: 'del_pro',
                content: '您确定要删除此仓库吗？<b>将删除该仓库的所有数据!</b> 不可恢复！',
                ok: function () {
                    $.ajax({
